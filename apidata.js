...
function get_mango() {
  fetch(`https://open.api.nexon.com/maplestory/v1/character/stat?ocid=fe370aba52d36e65175261eaa087a9d2efe8d04e6d233bd35cf2fabdeb93fb0d`, {
    method: 'GET',
    headers: {
      "x-nxopen-api-key": apikey
    }
    ...
    .then(res => res.json())
    .then(myJson => {
      const getmango = myJson.final_stat[42].stat_value;
      mango_force = Number(getmango);
      document.getElementById("mggij").innerHTML = `${numberToKorean(mango_force)}`;
    })
}
    ...
    .then(myJson => {
      ...
      fetch(req_url_status, {
        ...
        .then(myJson => {
          ...
          let result = myJson.final_stat[42].stat_value / mango_force;
          let result_kor = numberToKorean(myJson.final_stat[42].stat_value);
          if (fun_res >= 0) {
            charinfores2.innerHTML = `전투력 ${result_kor}<br>${(result).toFixed(1)} 망고`;
            if (textbox2.value == '황금맛포도') {
              charinfores2.innerHTML = `전투력 ${result_kor}<br>${(result).toFixed(1)} 망고<br>숯과자`;
            }
            if (textbox2.value == '원킬도적') {
              charinfores2.innerHTML = `전투력 ${result_kor}<br>${(result).toFixed(1)} 망고<br>응가도적`;
            }
            fun_res++;
          }
        })
